...
build_compute_node() {
    ...
    docker build \
        ...
        --tag "reg.optimus.id/chameleon/${IMAGE_NAME}-compute:live" \
        --file ${DOCKERFILE} \
        .
    docker push "reg.optimus.id/chameleon/${IMAGE_NAME}-compute:live"
...
build_web() {
    ...
    docker build \
        ...
        --tag "reg.optimus.id/chameleon/${IMAGE_NAME}-web:live" \
        ${BUILD_ARGS} \
        --file ${DOCKERFILE} \
        .
    docker push "reg.optimus.id/chameleon/${IMAGE_NAME}-web:live"
...
build_worker() {
    ...
    docker build \
        ...
        --tag "reg.optimus.id/chameleon/${IMAGE_NAME}-worker:live" \
        --file ${DOCKERFILE} \
        .
    docker push "reg.optimus.id/chameleon/${IMAGE_NAME}-worker:live"
