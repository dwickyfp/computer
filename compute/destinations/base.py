...
class BaseDestination(ABC):
    ...
    def test_connection(self) -> bool:
        """
        Test if destination connection is healthy without full initialization.

        This is a lightweight check used by DLQ recovery worker to determine
        if destination is reachable before attempting to replay messages.

        Returns:
            True if connection is healthy and destination is reachable
        """
        pass
