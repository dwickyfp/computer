...
export function useNodeSchema(
  ...
  const graphSnapshot = {
    ...
    edges: (edges ?? []).map((e) => ({
      ...
      source: e.source,
      target: e.target,
      sourceHandle: (typeof (e as Record<string, unknown>).sourceHandle ===
      'string'
        ? (e as Record<string, unknown>).sourceHandle
        : null) as string | null,
      targetHandle: (typeof (e as Record<string, unknown>).targetHandle ===
      'string'
  ...
  const enabled =
    !!flowTaskId && !!nodeId && (nodes ?? []).length > 0 && hasConfiguredInput
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey:
      flowTaskId && nodeId
        ? flowTaskKeys.nodeSchema(flowTaskId, nodeId, fingerprintKey)
        : ['node-schema', flowTaskId, nodeId, fingerprintKey],
