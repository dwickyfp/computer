...
import { ApiError } from '@/lib/handle-server-error'
...
function FlowCanvas({ flowTaskId }: { flowTaskId: number }) {
  ...
  const { data: ftResp } = useQuery({
    queryKey: flowTaskKeys.detail(flowTaskId),
  ...
  useEffect(() => {
    ...
    if (graphData) {
      ...
      if (Array.isArray(graph?.nodes_json))
        setNodes(graph.nodes_json as FlowNode[])
      else setNodes([])
      if (Array.isArray(graph?.edges_json))
        setEdges(graph.edges_json as FlowEdge[])
      else setEdges([])
      markClean()
  ...
  const { data: runStatusData } = useQuery({
    queryKey: flowTaskKeys.runStatus(pollingTaskId),
  ...
  useEffect(() => {
    ...
    if (
      runStatusData.state === 'SUCCESS' ||
      runStatusData.state === 'FAILURE'
    ) {
  ...
  // Poll preview task
  const [previewPollingTaskId, setPreviewPollingTaskId] = useState<
    string | null
  >(null)

  const { data: previewStatusData } = useQuery({
    queryKey: flowTaskKeys.previewStatus(previewPollingTaskId),
  ...
  useEffect(() => {
    ...
    if (previewStatusData.state === 'SUCCESS') {
      ...
      const currentSession =
        useFlowTaskStore.getState().preview.previewSessionId
  ...
  const runMutation = useMutation({
    ...
    onSuccess: (resp) => {
      setPollingTaskId(resp.celery_task_id)
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const nodeLabel = event.dataTransfer.getData(
        'application/reactflow-node-label'
      )
      if (!nodeType) return
  ...
  return (
    <div className='flex h-screen flex-col overflow-hidden'>
      {/* Top Bar */}
      <div className='z-10 flex flex-shrink-0 items-center gap-3 border-b border-border bg-background px-4 py-2'>
        <Breadcrumb>
          <BreadcrumbList>
            <BreadcrumbItem>
        ...
        <div className='ml-auto flex items-center gap-2'>
          ...
          {isDirty && (
            <Badge
              variant='outline'
              className='border-sky-500/40 bg-sky-500/10 text-[10px] text-sky-500'
            >
